 ...
 * ------------------------------------------------
 * The code reads all .txt files inside "text_pages" folder,
 * and searches for:
 *  1. Phone numbers
 *  2. Email addresses
 *  3. URLs
...
public class TextAnalyzer {
    ...
    // Define a folder name which containing text files
    ...
    private static void findAndWriteMatches(BufferedWriter writer, String label, Pattern pattern, String text)
        ...
        List<String> matcheList = new ArrayList<>();
        while (matcher.find()) {
            matcheList.add(matcher.group());
        }

        writer.write(label + " (" + matcheList.size() + " found):\n");
        if (matcheList.isEmpty()) {
            writer.write("  None found.\n");
        } else {
            for (String m : matcheList) {
                writer.write("  " + m + "\n");
            }
        }
        writer.write("\n");
